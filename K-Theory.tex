...
by identifying the points $\alpha,v)~(\alpha,f(\alpha),v)$ for $\alpha\in A$ 
...
The answer is affirmative over $\QQ$. For every vector bundle $E$ over $X$ we want to define $\ch(E)$ %as 
%\[
%ch(E) = ch_0(E) + ch_1(E) + \cdots
%\] 
...
\end{displaymath}
which shows that $\beta$ is surjective and also injective. Thus an isomorphism. Following the same lines, we see that $\tilde{K}^0(S^{2n})\to \tilde{K}^0(S^2\wedge S^{2n})$ is an isomorphism.
...
\begin{prop}
Suppose X is a compact space and $F$ is a vector bundle over $X\times S^2$ then there exist vector bundles $E_1, E_2, E_3$ such that
\[
...
Recall that a module $M$ over a ring $R$ is \emph{projective} if it is a direct summand of a free $R$-module. We will denote the set of isomorphism classes of finitely generated projective $R$-modules by $\proj(R)$. It is equipped with a direct sum $\oplus$ of modules, which makes it into a commutative monoid and we define $K_0(R)$ as the universal groupification of $\proj(R)$.
